...
class CollectionSelector:
    ...
    def __init__(self, execution_mode=2):
        ...
        # Align paths to .olaf/core layout
        self.core_dir = self.script_dir.parent.parent.parent  # .../core/skills/create-skill/scripts -> .olaf/core
        self.repo_root = self.core_dir.parent   # .olaf
        
        # Global paths (user-wide)
        from pathlib import Path
        home_dir = Path.home()
        self.global_core_dir = home_dir / ".olaf" / "core"
        ...
        self.local_collections_file = self.core_dir / "reference" / "local-competency-collections.json"
        self.local_competencies_dir = self.core_dir / "competencies"
        
        # Default to global collections, but support local override
        self.collections_file = self.global_collections_file
        self.competencies_dir = self.global_competencies_dir
        
        # Output index to local project's reference directory
        self.index_output = self.core_dir / "reference" / self.INDEX_FILENAME
    ...
    def load_manifest(self, skill_id: str) -> Dict:
        """Load skill manifest"""
        manifest_path = self.competencies_dir / skill_id / self.MANIFEST_FILENAME
        try:
    ...
    def generate_index(self, selected_competencies: List[str], collection_name: str) -> str:
        ...
        for competency_id in selected_competencies:
            ...
            for ep in entry_points:
                ...
                # Load skill manifest from .olaf/core/skills/{skill-id}/skill-manifest.json
                # The manifest path in competency is relative to core
                skill_manifest_file = self.core_dir / skill_manifest_path.lstrip('/')
...
"""
        ...
        for comp_id in selected_competencies:
            ...
            for ep in entry_points:
                ...
                if skill_location == "global":
                    full_path = f"~/.olaf/core/{skill_dir}/{prompt_path}"
                else:  # local
                    full_path = f"./.olaf/core/{skill_dir}/{prompt_path}"
    ...
    def _create_command_file(self, target_dir: Path, competency_name: str, comp_file_path: str, extension: str):
        ...
        # - "~/.olaf/core/skills/xxx/prompts/xxx.md" (global)
        # - "./.olaf/core/skills/xxx/prompts/xxx.md" (local)
...
"""
    ...
    def _select_custom_collection(self, collections: Dict, kernel_competencies: List[str]) -> tuple[List[str], str]:
        ...
        print("   To create a permanent collection, use: python .olaf/core/scripts/olaf-structure-management/create_collection.py")
