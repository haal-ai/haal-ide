...
|-----------|------------------|-------------------|---------------|
| **query-competency-index.md** | `~/.olaf/core/reference/` | ❌ READ-ONLY | Run `select_collection.py` |
| **olaf-framework-condensed.md** | `~/.olaf/core/reference/.condensed/` | ❌ READ-ONLY | Run condense-olaf-framework |
| **competency-manifest.json** | `.olaf/core/competencies/*/` | ✅ MODIFIABLE | Direct edit |
| **skill-manifest.json** (existing) | `.olaf/core/skills/*/` | ✅ MODIFIABLE | Direct edit |
| **skill-manifest.json** (new) | Creating new skill | ✅ MODIFIABLE | Generate new file |
| **Any file in new skill dir** | `.olaf/core/skills/[new-skill]/` | ✅ MODIFIABLE | Create new files |
...
1. **query-competency-index.md**
   - Location: `~/.olaf/core/reference/query-competency-index.md`
   - Generated by: `select_collection.py`
   - Purpose: Competency-to-skill mapping index
   - Update: Run `python .olaf/core/scripts/olaf-structure-management/select_collection.py --collection [name]`
...
```json
// File: .olaf/core/competencies/[competency-name]/competency-manifest.json
{
  "skills": [
    {
      "id": "new-skill",
...
Ready to run: python .olaf/core/scripts/olaf-structure-management/select_collection.py --collection [your-collection]? (yes/no)
```
...
**How to execute:**
```powershell
python .olaf/core/scripts/olaf-structure-management/select_collection.py --collection [collection-name]
```
...
```json
// File: .olaf/core/competencies/[target-competency]/competency-manifest.json
{
  "metadata": {
    "id": "developer",
...
```powershell
python .olaf/core/scripts/olaf-structure-management/select_collection.py --collection [user-collection]
```
...
```
⚠️  Script not found: .olaf/core/scripts/select_collection.py
...
4. Try running manually: python .olaf/core/scripts/olaf-structure-management/select_collection.py --collection all
```
...
- [ ] New skill directory created under `.olaf/core/skills/`
