            ...
            if not isinstance(bom["prompts"], list):
                ...
                for idx, prompt in enumerate(bom["prompts"]):
                    ...
                    if "path" not in prompt:
                        ...
                        # Check path is /skill.md
                        if prompt["path"] != "/skill.md":
                            errors.append(f"prompts[{idx}] path must be '/skill.md' (found: {prompt['path']})")
...
def main():
    ...
    schema_path = repo_root / "schemas" / "olaf-skill-manifest.schema.json"
